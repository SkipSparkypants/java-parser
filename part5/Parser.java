...
public class Parser {
	...
	private void statement_list() {
            while( is(TK.DO) || is(TK.ID) || is(TK.IF) || is(TK.TILDE) || is(TK.PRINT) | is(TK.FOR) ) {
                statement();
            }
	}
	...
	private void for1(){
		mustBePrint(TK.FOR, "for(");
		if (is(TK.ID) || is(TK.TILDE)){
			assignment(1);
		}
		mustBePrint(TK.NEXT, "; ");
		if (is(TK.ID) || is(TK.TILDE) || is(TK.NUM) || is(TK.LPAREN)){
			factor();
			comp();
			factor();
		}
	...
	private void initProgram(){
        ...
        private void multOp() {
            if( is(TK.TIMES) ){
                mustBePrint(TK.TIMES, " * ");
            }
            else if( is(TK.DIVIDE ) ){
				mustBePrint(TK.DIVIDE, "/");
            }
			else
				parse_error("multOp");
        }		
